...
// defaul fraction to bottom of 0.4 in case FRACTION_TO_BOTTOM is not set
const fractionToExit = Number(process.env.FRACTION_TO_BOTTOM) || 0.4;
...
const routine = async () => {
  ...
  try {
    await liquidityManager.manage(fractionToExit, {
      test: false,
      inverse: false,
      autoUSDCQuote: true,
      counterDump: false,
